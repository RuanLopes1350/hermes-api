...
const PG_ERROR_CODES: Record<string, { status: number; message: string }> = {
	'23505': {
		status: HttpStatusCode.CONFLICT.code,
		message: 'Já existe um registro com esse valor',
	},
	'23503': {
		status: HttpStatusCode.UNPROCESSABLE_ENTITY.code,
	...
	'23514': {
		status: HttpStatusCode.UNPROCESSABLE_ENTITY.code,
		message: 'Valor não atende às restrições do banco de dados',
	},
	'22001': {
		status: HttpStatusCode.UNPROCESSABLE_ENTITY.code,
	...
	'57014': {
		status: HttpStatusCode.INTERNAL_SERVER_ERROR.code,
		message: 'Consulta cancelada por tempo limite excedido',
	},
};
	...
	for (const candidate of candidates) {
		if (candidate instanceof PgDatabaseError) {
			...
			if (pgCode && PG_ERROR_CODES[pgCode]) {
				...
				console.error(`[DatabaseError] [${context}] PG ${pgCode} - ${message}`, {
					detail,
					constraint,
				});
			...
			console.error(
				`[DatabaseError] [${context}] PG código desconhecido (${pgCode}):`,
				candidate.message,
			);
