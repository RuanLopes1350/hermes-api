...
class CredentialService {
	...
	async createCredential(serviceId: string, data: unknown, userId: string) {
		...
		if (!serviceExists)
			throw new CredentialDomainError('Serviço não encontrado.', 404, 'SERVICE_NOT_FOUND');
		...
		console.log(
			chalk.magenta(
				`[${getTimestamp()}] [INFO] [CredentialService] Gerando API Key vinculada à nova credencial...`,
			),
		);
	...
	async listCredentials(serviceId: string, userId: string) {
		const serviceExists = await serviceRepository.findByIdAndOwner(serviceId, userId);
		if (!serviceExists)
			throw new CredentialDomainError('Serviço não encontrado.', 404, 'NOT_FOUND');
